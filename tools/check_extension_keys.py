...
relation. Measured the day it was retired, with the core schema untouched around it:
    ...
    the main fact view         declared 0 multi-row (VERIFIED 2026-08-16)  ->  millions multi-row
    a second fact view         declared 0 multi-row, 0 divergent           ->  nearly every cell multi-row
    a reach view               declared 0 ambiguous                        ->  0. HOLDS.
    a current-snapshot view    declared its cell count                     ->  reproduced (+116). HOLDS.

Two of four false — and WHICH two is the interesting half. Both survivors are views that ENFORCE the
key: the reach view withholds ambiguous cells, and the current-snapshot view collapses to one row per cell,
