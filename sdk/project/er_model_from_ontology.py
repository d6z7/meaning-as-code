#!/usr/bin/env python3
"""sdk.project.er_model_from_ontology — the ONTOLOGY plane's relationship builder. NOT WIRED.

WHAT THIS FILE IS, AND WHY IT IS NOT THE ER DIAGRAM ANY MORE
-------------------------------------------------------------
This module builds an entity-relationship picture out of `ontology/edges.yaml`. That is what the
"ER Diagram" under the DATA group used to draw, and it was a conflation: an ontology edge relates
BUSINESS OBJECTS, a physical reference relates RELATIONS AND COLUMNS, and the two were sharing one
artifact. Twenty-five entries on a live bundle declared `level: physical, type: foreign_key` while
being keyed by `endpoints.from.concept`, with the relations and columns flattened into a
`join_rule` STRING that this file had to re-parse with a regular expression (`_parse_join`, below).

The consequences were measured, not theoretical:
  * a bundle with no ontology could not draw a physical diagram AT ALL, however completely its
    warehouse had been measured — a foreign key needed a concept to exist first;
  * every crow's foot was an AUTHORED claim, so an optional relationship and a mandatory one drew
    identically wherever the author had not thought about participation;
  * a reference whose two columns are spelled differently was unreachable, because a concept-keyed
    entry is written by somebody who already believes the two things are the same thing.

`sdk.project.er_model` now builds the PHYSICAL diagram from `data/references/*.yaml`, measured
through the connector seam, and reads nothing under `ontology/`.

WHY THIS FILE IS KEPT RATHER THAN DELETED, said plainly so nobody re-wires it by accident:
THREE DIAGRAMS WERE RULED, not one. (1) the physical ER — relations, columns, keys, references,
cardinality, participation, measured from the warehouse; (2) the ONTOLOGY graph — business objects
and their relationships; (3) GROUNDING — which concept rests on which relation. This module is
(2)'s machinery, and (2) is not built here. NOTHING IN PRODUCTION IMPORTS THIS FILE — that is
checkable with a grep and it is meant to stay true until the ontology diagram's author wires it.
It is not a fallback for the physical diagram, and re-introducing it as one re-introduces the
defect above.

It is unchanged below except for this header and for `CARDINALITY` and `_components`, which are
plane-neutral and are imported from `er_model` rather than copied — a second home for the closed
crow's-foot vocabulary is exactly the drift this split exists to remove.
...
# The crow's-foot vocabulary and the connected-components metric are PLANE-NEUTRAL and live in
# `er_model`, the module that is wired. Imported rather than copied: a second home for a closed
