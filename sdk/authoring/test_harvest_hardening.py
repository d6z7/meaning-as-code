...
def test_canonicalization_aligns_stem_tablename_and_relation():
    ...
    view_schema, raw_table = "fpl2", "dim_country"
    doc = {
        "dataset": {
...
def test_persist_extracts_sql_to_sibling_and_leaves_only_pointer(tmp_path):
    ...
    stem = "dim_country"
    body = (
        "CREATE OR REPLACE VIEW fpl2.dim_country AS\n"
        "SELECT fpl_brand_country_code, market, iso2\n"
    ...
    files = {
        "transform": {
            "obj": {
                ...
                "produces": {"relation": f"fpl2.{stem}", "grain": "one row per country"},
...
def _make_source(root: Path, view_schema="fpl2") -> Path:
    cr = _declare_bundle(root / "sources" / "gaps" / "fpl2")
        ...
        yaml.safe_dump(
            {
                ...
                "columns": [
                    {"name": "fpl_brand_country_code", "role": "primary_key"},
...
def test_plan_materialization_builds_own_schema_ddl_no_aws(tmp_path):
    cr = _make_source(tmp_path, "fpl2")
    plan = materialize.plan_materialization(cr)  # PURE — no Athena
    assert plan["view_schema"] == "fpl2"
    v = plan["views"][0]
    ...
    assert v["verify_sql"] == "SELECT 1 FROM fpl2.dim_country LIMIT 1"


def test_materialize_refuses_shared_and_empty_schema(tmp_path):
    with pytest.raises(materialize.MaterializeRefused):
        materialize.plan_materialization(_make_source(tmp_path / "shared", "fpl"))  # gold/shared
    with pytest.raises(materialize.MaterializeRefused):
...
def test_run_materialize_execute_drives_injected_executor_no_athena(tmp_path):
    ...
    assert any(s.startswith("SHOW TABLES IN fpl2") for s in recorded)  # collision-check first
    assert any(s.startswith("CREATE OR REPLACE VIEW fpl2.dim_country AS") for s in recorded)
    assert "SELECT 1 FROM fpl2.dim_country LIMIT 1" in recorded  # verified queryable
...
def test_plan_lookups_builds_name_to_code_sql_no_aws(tmp_path):
    ...
    assert lk["source_view"] == "fpl2.dim_country"
    assert lk["sql"] == (
        'SELECT DISTINCT "market" AS name, "iso2" AS "iso2" '
        "FROM fpl2.dim_country "
