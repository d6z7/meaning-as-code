...
| [mac.schema.json](mac.schema.json) | The **formal, machine-checkable schema** (v0.1.8) — the single source of structural truth: closed vocabulary, class/level/type/role enums, required keys, and the `x-` extension rule. |
| [CONFORMANCE.md](CONFORMANCE.md) | Conformance levels (L0–L3), the closed-core + `x-` extension contract, and the v0.1.8 change list. |
| [tools/validate_schema.py](tools/validate_schema.py) | The **structural** validator — schema-driven (MAC v0.1.8): checks every model file against `mac.schema.json` (closed vocabulary, required keys, naming contract, edge legality). |
...
#   enforces files at the current schema_version (0.1.8) and skips the rest; --all checks everything, --strict fails on warnings
...
The framework and its file-format schema share one version, **v0.1.8** — a machine-checkable contract
