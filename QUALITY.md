...
(e.g. GAPS/FPL), keep the feature branch open as a long-lived integration branch: applied-instance work
...
7. **Validate on an applied instance, not only the examples.** Before a change is pushable, exercise it on
   a real applied ontology (e.g. GAPS **FPL**): regenerate its projections and run its gates against the
